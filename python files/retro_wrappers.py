...
from wrappers import TimeLimit, ActTimeLimit
import numpy as np
import gym

'''
original file:https://github.com/openai/baselines/blob/master/baselines/common/retro_wrappers.py
'''
...
class TerminalOnLifeLossWrapper(gym.Wrapper):
    '''
    TerminalOnLifeLossWrapper is a wrapper that ends the episode when Sonic dies.
    '''
    def __init__(self, env):
        gym.Wrapper.__init__(self, env)
        self.x = None
    ...
    def step(self, action):
        ...
        if info['lives'] < 3:
            game_over = True
            reward += -20. #reward when Sonic dies.
        if self.act is not info['act']: #reset some values when stage clear.
            self.zone = info['zone']
            self.act = info['act']
            self.x = info['screen_x']
        else: #reward when Sonic go forward.
            reward += 1e-2 * (info['screen_x'] - self.x)
            self.x = info['screen_x']
        return observation, reward, game_over, info

class GameOverWrapper(gym.Wrapper):
    '''
    GameOverWrapper is a wrapper to confirm that the game is over.
    '''
    def __init__(self, env):
        gym.Wrapper.__init__(self, env)
...
def make_retro(*, game, state=None, max_episode_steps=5500, terminal_on_life_loss=True, **kwargs):
    import retro
    if state is None:
    ...
    if max_episode_steps is not None:
        env = ActTimeLimit(env, max_episode_steps=max_episode_steps)
