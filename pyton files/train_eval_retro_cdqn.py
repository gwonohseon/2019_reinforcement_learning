...
r"""Train and Eval Categorical DQN on Sonic environments.
...
This program is for learning Sonic. Categorical dqn is used as the algorithm.
This code is a modification of the code that trains Atari with tf_agents cdqn.

For a good survey of training on Atari, see Machado, et al. 2017:
https://arxiv.org/pdf/1709.06009.pdf.

original file: https://github.com/tensorflow/agents/blob/master/tf_agents/agents/categorical_dqn/examples/train_eval_atari.py
To run:

```bash
sonic/train_eval_retro_cdqn \
 --root_dir=$HOME/logs/sonic_cdqn
```
...
flags.DEFINE_string('root_dir', 'logs/sonic_cdqn',
                    'Root directory for writing logs/summaries/checkpoints.')
flags.DEFINE_string('game_name', 'SonicTheHedgehog-Genesis', 'Name of Atari game to run.')
...
class RetroCategoricalQNetwork(categorical_q_network.CategoricalQNetwork):
  ...
  def call(self, observation, step_type=None, network_state=None):
    ...
    state = state / 255
    return super(RetroCategoricalQNetwork, self).call(
...
class TrainEval(object):
  ...
  def __init__(
      ...
      env_name,
      num_iterations=200,
      # max_episode_frames=240000,  # ALE frames
    ...
    """A simple Retro train and eval for CDQN.

    Args:
      root_dir: Directory to write log files to.
      env_name: Fully-qualified name of the Retro environment (i.e. Pong-v0).
    ...
    with tf.compat.v2.summary.record_if(
      ...
      with tf.device('/cpu:0'):
        ...
        epsilon = tf.compat.v1.train.polynomial_decay(
            1.0,
            self._global_step,
            epsilon_decay_period / self._update_period,
            end_learning_rate=epsilon_greedy)
      ...
      with tf.device('/gpu:0'):
        optimizer = tf.compat.v1.train.RMSPropOptimizer(
            ...
            epsilon=0.00001,
            centered=True)
        categorical_q_net = RetroCategoricalQNetwork(
            observation_spec,
            action_spec,
        ...
        agent = categorical_dqn_agent.CategoricalDqnAgent(
            ...
            target_update_period=(
                    target_update_period / self._update_period),
            gamma=gamma,
  ...
  def game_over(self):
      return self._env.envs[0].gym.game_over
...
def main(_):
  ...
  TrainEval(FLAGS.root_dir, FLAGS.game_name,
            **get_run_args()).run()
